...
const FAN_POSITIONS = [
  { rot: -14, scale: 0.82, x: -30, y: 4.0, zIndex: 1 },
  { rot: -9, scale: 0.89, x: -18, y: 2.0, zIndex: 2 },
  { rot: -4, scale: 0.96, x: -8, y: 0.6, zIndex: 3 },
...
function CardFanCarousel({ cards, onSelectImage }) {
      ...
      visibleEntries.forEach(({ el, slot }) => {
        ...
        if (hoveredSlot !== null) {
          ...
          if (slot === hoveredSlot) {
            ...
            const pushStrength = 12 * (1 - Math.abs(normalized)) * (1 + 0.2 * Math.max(0, 3 - distance))
  ...
  return (
    <div className="fan-carousel">
      <div className="fan-carousel__inner">
        <div ref={containerRef} className="fan-carousel__layout">
          {cards.map((card, index) => (
            <div
              key={index}
              className="fan-card"
              onClick={() => onSelectImage?.(card)}
            >
              <img
                src={card.imgUrl}
                loading="lazy"
      ...
      {needsPagination && (
        <div className="fan-carousel__controls">
          <button className="fan-carousel__arrow" onClick={() => cycle('left')} aria-label="Previous">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <polyline points="15 18 9 12 15 6" />
            </svg>
          </button>
          <div className="fan-carousel__dots">
            {cards.map((_, i) => (
              <span key={i} className={`fan-carousel__dot ${i === centerIndex ? 'fan-carousel__dot--active' : ''}`} />
            ))}
          </div>
          <button className="fan-carousel__arrow" onClick={() => cycle('right')} aria-label="Next">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
